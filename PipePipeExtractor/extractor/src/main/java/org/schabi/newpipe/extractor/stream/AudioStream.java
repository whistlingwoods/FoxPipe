...
public final class AudioStream extends Stream {
    ...
    public static final class Builder {
        ...
        public Builder setAudioLocale(@Nullable final Locale audioLocale) {
            this.audioLocale = audioLocale == null ? null : audioLocale.toLanguageTag();
            return this;
        }
    ...
    public Locale getAudioLocale() {
        if (audioLocale == null || audioLocale.isEmpty()) {
            return null;
        }
    ...
    @Nullable
    public String getAudioLocaleTag() {
        return audioLocale;
    }

    @Nullable
    public AudioTrackType getAudioTrackType() {
        final String source = (audioTrackName == null ? "" : audioTrackName)
                + " "
                + (audioTrackId == null ? "" : audioTrackId);
        final String lowerCaseSource = source.toLowerCase(Locale.ROOT);
        if (lowerCaseSource.contains("descriptive")) {
            return AudioTrackType.DESCRIPTIVE;
        }
        if (lowerCaseSource.contains("secondary")) {
            return AudioTrackType.SECONDARY;
        }
        if (lowerCaseSource.contains("dubbed") || lowerCaseSource.contains("dub")) {
            return AudioTrackType.DUBBED;
        }
        if (lowerCaseSource.contains("original")) {
