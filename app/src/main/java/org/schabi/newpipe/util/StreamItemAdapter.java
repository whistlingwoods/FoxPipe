...
public class StreamItemAdapter<T extends Stream, U extends Stream> extends BaseAdapter
    ...
    public StreamItemAdapter(
        ...
        this(streamsWrapper, secondaryStreams, null);
    }
    ...
    public StreamItemAdapter(@NonNull final StreamInfoWrapper<T> streamsWrapper,
                             @Nullable final Context context) {
        this(streamsWrapper, new SparseArrayCompat<>(0), context);
    }
    ...
    public View getView(final int position, final View convertView, final ViewGroup parent) {
        final int selectedPosition = parent instanceof Spinner
    ...
    private View getCustomView(final int position,
        ...
        if (stream instanceof VideoStream) {
            ...
            if (audioStream.getAverageBitrate() > 0) {
                ...
                qualityString = parentContext.getString(R.string.unknown_quality);
            }
        } else if (stream instanceof SubtitlesStream) {
            qualityString = ((SubtitlesStream) stream).getDisplayLanguageName();
            if (((SubtitlesStream) stream).isAutoGenerated()) {
                qualityString += " ("
                        + parentContext.getString(R.string.caption_auto_generated) + ")";
            }
        } else {
            if (mediaFormat == null) {
                qualityString = parentContext.getString(R.string.unknown_quality);
        ...
        if (stream instanceof SubtitlesStream) {
            ...
            if (mediaFormat == null) {
                formatNameView.setText(parentContext.getString(R.string.unknown_format));
    ...
    public CharSequence getDisplayLabel(final int position) {
        final Context labelContext = context;
        ...
        final T stream = getItem(position);
        final MediaFormat mediaFormat = streamsWrapper.getFormat(position);
        final java.util.ArrayList<String> parts = new java.util.ArrayList<>(3);

        if (stream instanceof VideoStream) {
            parts.add(((VideoStream) stream).getResolution());
        } else if (stream instanceof AudioStream) {
            final AudioStream audioStream = (AudioStream) stream;
            if (audioStream.getAverageBitrate() > 0) {
                parts.add(audioStream.getAverageBitrate() + "kbps");
            } else {
                parts.add(labelContext.getString(R.string.unknown_quality));
            }
        } else if (stream instanceof SubtitlesStream) {
            final SubtitlesStream subtitlesStream = (SubtitlesStream) stream;
            parts.add(subtitlesStream.getDisplayLanguageName());
            if (subtitlesStream.isAutoGenerated()) {
                parts.add(labelContext.getString(R.string.caption_auto_generated));
            }
        } else {
            parts.add(labelContext.getString(R.string.unknown_quality));
        }
        ...
        if (stream instanceof SubtitlesStream) {
            parts.add(((SubtitlesStream) stream).getLanguageTag());
        } else if (mediaFormat == null) {
            parts.add(labelContext.getString(R.string.unknown_format));
        } else if (mediaFormat == MediaFormat.WEBMA_OPUS) {
            parts.add("opus");
        } else {
            parts.add(mediaFormat.getName());
        }

        if (streamsWrapper.getSizeInBytes(position) > 0) {
            final var secondary = secondaryStreams.get(position);
            final long size = secondary == null
                    ? streamsWrapper.getSizeInBytes(position)
                    : secondary.getSizeInBytes() + streamsWrapper.getSizeInBytes(position);
            parts.add(Utility.formatBytes(size));
        }
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            protected FilterResults performFiltering(final CharSequence constraint) {
                ...
                filterResults.values = streamsWrapper.getStreamsList();
            ...
            public CharSequence convertResultToString(final Object resultValue) {
                final int index = streamsWrapper.getStreamsList().indexOf(resultValue);
